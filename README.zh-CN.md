...
- **菜单栏控制**：状态栏菜单里可以设置登录时启动、系统 Dock 唤醒时间、钨极自己的唤醒时间。
...
- **`唤醒系统 dock栏`**：调整系统原生 Dock 的唤醒等待时间。拖动时会先保存到钨极偏好；松手且数值确实变化后，会直接应用到系统 Dock，并重启系统 Dock 让设置生效。
- **`唤醒 Tungsten Edge 钨极`**：调整鼠标停在底边多久后唤醒钨极。`常驻` 表示一直显示，有限值范围是 `0.1s` 到 `3.0s`，`不唤醒` 表示仍会自动隐藏，但底边不唤醒。
...
之后可以在钨极的状态栏菜单里调整系统 Dock 的唤醒等待时间。
