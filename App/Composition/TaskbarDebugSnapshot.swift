...
// 「导出任务条调试快照」的 JSON 报告模型与写盘。只被 `AppRuntime.exportDebugSnapshot` 调用；
// 2026-09-05 从 AppComposition.swift 搬出，与运行时本体无关。

// MARK: - Debug Snapshot Export

enum TaskbarDebugSnapshotExporter {
...
struct TaskbarDebugReport: Codable {
...
struct TaskbarDebugCard: Codable {
    let order: Int
    let id: String
    let title: String
    let status: String
...
struct TaskbarDebugDuplicateGroup: Codable {
    let key: String
    let count: Int
    let ids: [String]
    let titles: [String]
...
struct TaskbarDebugLiveWindow: Codable {
    let source: String
    let pid: Int32
    let bundleIdentifier: String?
    let title: String?
...
struct TaskbarDebugRect: Codable {
    let x: Double
    let y: Double
    let width: Double
    let height: Double
