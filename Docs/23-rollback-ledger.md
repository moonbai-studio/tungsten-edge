...
以下结论以 **2026-07-20 的 `418060d`**（开发线最新检查点：忽略本地临时产物）为起点。**2026-07-20 起 `codex/release-v0.6.6` 与 `finder-folder-preview` 已合并对齐指向同一提交**，旧开发线原始历史保存在标签 `backup/finder-folder-preview-20260720`（其中 `0a70130` / `7ec5264` 两个功能提交已分别回摘为 `0b74fa6` / `a18c8aa`，撤销请用新 hash）；其下的三键数据边界见下方专节，更早的抽屉位置 / 保留拆分数据边界仍适用于 `5f5efa0`。旧提交即使产品上是独立能力，也可能因为后来改过同一文件而无法直接 `git revert`；“人工”表示要解决冲突并重新验证，不表示功能不能移除。
...
| 目标 | 建议逆序 | 会保留什么 | 当前实证 |
| --- | --- | --- | --- |
| **撤销忽略本地临时产物** | `git revert 418060d` | 全部功能；`default.profraw` / `output/` / `tmp/` 恢复为未跟踪可见状态 | **↩ 单点，无代码影响**：仅 `.gitignore` 加 5 行；从旧开发线回摘（原 `625691a`）。发布分支此前一直缺这条，故临时产物长期裸露在 `git status` 里 |
| **撤销未启动应用「打开」菜单项 + 消息区退出态灰显** | `git revert a18c8aa` | 最小化命令分离及此前全部功能；恢复未启动图标右键无「打开」项、消息区退出态常亮（不灰显）的旧观感 | **↩ 单点，无数据边界**：纯 UI/菜单层——纯 `LauncherMenuPlan.open` + `LauncherChipVisualPlan`（均单测），`dimsWhenInactive`→`dimsWhenHidden` 重命名。从旧开发线回摘（原 `7ec5264`，owner 已在旧线实机验收：右键「打开」重启、消息退出图标灰显、抽屉右键打开不误关）；471 项单测、Debug 构建通过；未单独验证 revert |
| **撤销启停首帧影子滑动修复** | `git revert 0b74fa6` | 最小化命令分离及此前全部功能；恢复退出/重开首帧新卡先落 live 区尾部、再被 spring 拉回原位的旧观感（「重开跳位」复现） | **↩ 单点，无数据边界**：纯排序层——`StripOrderStore` 的缺席 rank-anchor 被 `reconciled`（渲染）与 `sync`（副作用）共享，首帧即落原位；含 6 项 pre-sync 首帧回归 + AGENTS 护栏同一提交。从旧开发线回摘（原 `0a70130`，owner 已在旧线实机验收）；471 项单测、Debug 构建通过；未单独验证 revert |
| **撤销最小化命令提交与状态确认分离** | `git revert df482d3` | SkyLight make-key 修复及此前全部功能；恢复「同步 AX 读回决定最小化成败」与失败后反向聚焦，**Illustrator 类异步最小化应用会再次自动还原** | **↩ 单点，无数据边界**：`minimize` 的 success 改为「命令已提交」、`verifiedMinimized` 降级为诊断；删除失败后反向聚焦；`skipEarlyFocus`→`forceRestoreBeforeFocus` 并入 `activate()` 既有恢复分支；新增 `BackgroundFocusHandoff` 纯决策 + 单测。467 项单测、主 App 与 window-lab 构建、owner 实机验收（Illustrator 最小化不再自动还原）通过；未单独验证 revert |
