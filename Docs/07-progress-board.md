...
- AX 路径已开始桥接 `CGWindowID`；2026-05-15 万联、Terminal、Chrome 短测未复发重复卡。
...
- AX 观察路径当前会尽量带上 `CGWindowID`，让 AX / CG 两条证据更容易落到同一个窗口主键；这不改变 Finder / Feishu 策略，也不引入 slot policy。
...
- 2026-05-15 当前工作区已补 AX -> `CGWindowID` 桥接，让 inventory、普通 accessibility、Finder AX 观察能把系统窗口编号传入既有 identity 逻辑。
...
- 2026-05-15 AX -> `CGWindowID` 桥接版短测：万联最小化 10 秒恢复同一张卡 ID 不变，Terminal 改标题不新增重复卡，Chrome 两窗口稳定显示 2 张卡；本轮快照 `ax-transient = 0`，历史旧快照曾出现 27 / 34 级别。
...
- 2026-05-15 真实桌面短测已覆盖 AX -> `CGWindowID` 桥接版的万联最小化恢复、Terminal 标题变化、Chrome 双窗口稳定性。
...
- 真实桌面验收：
  ...
  - AX -> `CGWindowID` 桥接只完成短测，还需要长时间实机验证
  - 还需要继续过夜 / 睡眠 / 长时间空闲回归
  - 继续确认正常用户窗口完整进入任务条
...
1. 继续运行 AX -> `CGWindowID` 桥接版正式 app 做真实桌面回归，重点看长时间空闲 / 睡眠 / 过夜后是否还会出现重复卡片。
2. 如果再次出现重复，不要重启或清理，立即导出调试快照：
   - Debug 菜单：`导出任务条快照`
   - 快捷键：`Cmd+Shift+D`
   - 或 Debug 构建下对进程发送 `SIGUSR2`
3. 若桥接版长测仍复发，再做最小范围 slot policy 验证；暂不做全 App 分类重构。
4. 若长时间重复卡片问题不再复现，继续做 inventory-first 真实桌面可信度验收，并再决定浏览器窗口粒度和抽屉策略。
