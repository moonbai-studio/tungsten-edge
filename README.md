...
- **Show taskbar on ▸** — appears only with two or more displays. The default is **Follow the mouse**; you can also pin the taskbar to one screen, after which resting the pointer at another screen's bottom edge no longer moves it. Unplug the pinned screen and the bar falls back to the main display, returning when you plug it back in.
