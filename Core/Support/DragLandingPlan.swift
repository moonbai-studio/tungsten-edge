...
enum DragLandingPlan {
    ...
    /// 松手之后，归位飞行**先按住多久再起飞**（只作用于飞回卡槽/格子，吸进胶囊不等）。
    ///
    /// 松手那一刻落点锚点还是**落定之前**那一帧的：抽屉网格要等这次投放提交后的布局才知道
    /// 这一格最终落在第几列。不等就起飞，CA 会先朝旧格子飞出去，几毫秒后 `retargetLandingIfNeeded`
    /// 再把终点拽回来——屏幕上就是一条折线（owner 2026-08-20：「先飘到目标位置偏右，再去到目标位置」）。
    ///
    /// **按时间等，不按 run loop 轮次**，理由同 `pickUpSettle`：真机鼠标 1–8ms 一个事件，
    /// 下一轮往往还在同一帧里。两个显示帧足够等到那次布局；这段时间载体就停在松手位置，
    /// 飞行本身 320–500ms，多这两帧看不出来。
    static let landingSettle: TimeInterval = 0.034
