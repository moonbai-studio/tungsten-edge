...
- **Precise insertion (strip→open-drawer running zone, 2026-06-21):** dropping over the open drawer lands the app at the cursor slot, not the end. `DrawerView` computes the running-zone insertion index from `dragController.globalLocation` mapped into the `"drawer"` space via **AppKit** `convert(bounds,to:nil)`+`convertToScreen` (a `ScreenRectReader`) — **not** SwiftUI `.global` (y-flip/shadowPadding traps). It publishes a global order index via `setDropPreview` (deduped, from `onChange`, never in `body`). Hit-test uses a **frozen snapshot** of the pre-gap chip frames (`baseRunningFrames`) so the inserted placeholder's shift doesn't make the hit jump. `endDrag` **captures the preview before `teardown`**, then commits via `DrawerOrderStore.insert(at:)` **then** `drawerStore.add` (order-before-member, else sync appends-to-end first). Preview only over the open drawer (drawer-closed capsule drop = append). The gap placeholder is transparent and gated on an active drag (clears same frame as the real chip lands → no twitch). Same-zone only: a strip drag (always a running app) previews only the **running** zone.
- **Spring-load (2026-06-21):** a strip drag hovering the capsule with the drawer closed auto-opens it after ~0.4s, then precise insertion takes over. The timer **must** use `RunLoop.main.add(timer, forMode: .common)` — a `scheduledTimer` (default mode) never fires during drag event-tracking. After opening, call `dragController.bringCarrierToFront()` (the freshly-ordered drawer can cover the carrier).
